classdef WaveletTransformer < handle
    ...
    methods (Access = public)
        function obj = WaveletTransformer(cParams)
            obj.init(cParams);
        end
    ...
    methods (Access = private, Static)
        ...
        function  wt2D(obj,data)
            switch obj.transmethod
                case 'matlab'
                    obj.winfo.L = wmaxlev(size(data),obj.winfo.motherwave);
                    [obj.wave,obj.winfo.l] = wavedec2(data,obj.winfo.L,obj.winfo.motherwave);
                case 'dwt_matlab'
                    [A,H,V,D] = dwt2(data,obj.winfo.motherwave);
                    obj.wave(:,:,1) = A;
                    obj.wave(:,:,2) = H;
                    obj.wave(:,:,3) = V;
                    obj.wave(:,:,4) = D;
                case 'cwt_matlab'
                    obj.wave = cwtft2(data);
                    cprintf('err', 'This method does not perform compression  \n');
                case 'dwt'
                    [A,H,V,D] = obj.dwt_2D(data,obj.winfo.motherwave);
                    obj.wave(:,:,1) = A;
                    obj.wave(:,:,2) = H;
                    obj.wave(:,:,3) = V;
                    obj.wave(:,:,4) = D;
                ...
                case 'packet'
                    ...
                    %obj.draw_packets(obj.wave,obj.winfo.par.N,obj.winfo.par.pdep,obj.winfo.s,obj.winfo.packet_stream);
            end
        end
        ...
        function signal = iwt2D(obj,data)
            switch obj.transmethod
                case 'matlab'
                    signal = waverec2(data,obj.winfo.l,obj.winfo.motherwave);
                case 'dwt_matlab'
                    A = data(:,:,1);
                    H = data(:,:,2);
                    V = data(:,:,3);
                    D = data(:,:,4);
                    signal = idwt2(A,H,V,D,obj.winfo.motherwave);
                case 'cwt_matlab'
                    cprintf('err', 'This method cannot perform inverse and reconstruction (Only for direct transform)  \n');
                case 'dwt'
                    A = data(:,:,1);
                    H = data(:,:,2);
                    V = data(:,:,3);
                    D = data(:,:,4);
                    signal = obj.idwt_2D(A,H,V,D,obj.winfo.motherwave);
                    
                case {'wavedec_matlab','multilevel_matlab'}
                    signal = obj.waveletRec4Images(data, obj.winfo, obj.winfo.motherwave);
                case 'packet'
                    signal = obj.recon_packets2D(data,obj.winfo.par,obj.winfo.packet_stream);
