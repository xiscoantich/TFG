function hn=wavelet_fun(wavelet,n,type,plt)
%Computes (and plots) samples of the wavelet function 
%hn=wavelet_fun(wavelet,n,type,plt)
%
%Input:
...
%        if (plt == 'plot') then plots the wavelet function using 'plot'
%        if (plt == 'stairs') then plots the wavelet function using 'stairs'
%        Note that the stem plot is used if n is 0.
%
%Output:
% hn - n-th approximation of the wavelet function
%
%Note:
% If n is 0, then use stem instead of plot. Also, if output defined, does
% not plot at all.
% For some reason, the symbol \psi is not displayed in the Figure window, 
% but appears when exported to .eps or some other format.
%
%Uses:
% load_wavelet.m
%
%Example: 
% wavelet_fun('CDF_9x7',5,'r');
% g = wavelet_fun('LeGall_5x3',7,'d','plot');
...
if (type == 'd')
    hl = wvf.filt_H0;
    hh = wvf.filt_H1;
    delay = wvf.filt_H1_delay;
    figstr = [waveletstr ' analysis, '];      
elseif (type == 'r')
    hl = wvf.filt_G0;
    hh = wvf.filt_G1;
    delay = wvf.filt_G1_delay;    
...
if (n > 0)
    ...
    for i=1:n
        hnn = zeros(1,max(length(hn)*2 - 1,1));
        hnn(1:2:end) = hn;
        hn = conv(hnn,hl);
    end;
else
...
if ~isempty(plt)
    ...
    if (n > 0)
        ...
        if (type == 'd')
            titlstr = '$$\tilde{\psi}(t)$$';
        elseif (type == 'r')
            titlstr = '$$\psi \; \, (t)$$'; %space is added to comepnsate for bug when saving into eps
        end;
